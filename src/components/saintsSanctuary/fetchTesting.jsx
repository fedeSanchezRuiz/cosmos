...
const BronzeSaints = () => {
  ...
  const [bronzeSaints, setBronzeSaints] = useState([])

  async function fetchBronzeSaintsHandler() {
    const response = await fetch('https://cosmos-92e61-default-rtdb.firebaseio.com/bronze.json')
    const data = await response.json();
    
      setBronzeSaints(data.results);
  }
